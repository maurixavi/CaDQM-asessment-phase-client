...
export class DqMeasurementResultsComponent implements OnInit {
  ...
  subscribeToData(): void {
    ...
    this.projectDataService.dqModelVersion$.subscribe((dqModelVersionId) => {
      ...
      if (this.dqModelVersionId) {
        this.fetchDQModelMeasurementExecutions();
      }
    });
  ...
  onCompleteStage(): void {
    this.router.navigate(['/phase2/st6/assessment-approaches']); 
    this.currentStep = 0;
  }
  ...
  fetchDQModelMeasurementExecutions(): void {
    ...
    this.modelService.getAllDQModelExecutions(this.dqModelVersionId).subscribe({
      next: (response: any) => { 
        ...
        console.log("this.measurementExecutions", this.measurementExecutions);
        ...
        if (this.measurementExecutions.length > 0) {
          ...
          if (this.selectedExecution) {
            console.log("this.selectedExecution", this.selectedExecution)
            //Habilitar NextStage si finalizo DQ Model Execution
            /*if (this.selectedExecution.completed_at !== null){
  ...
  onExecutionChange(): void {
    ...
    this.modelService.getSpecificDQModelExecution(
      ...
      next: (executionDetails) => {
        ...
        //Habilitar NextStage si finalizo DQ Model Execution
        if (executionDetails.completed_at){
          this.isNextStepEnabled = true;
        }
